...
namespace BollettiniMonitoraggio
{
    class AllA
    {
        ...
        protected System.IO.StreamReader fileCSV1;
        protected System.IO.StreamReader fileCSV3;
        ...
        protected string filenameTmp; // nome file pdf da usare finchè non aggiungo i campi compilabili nell'header
        ...
        public AllA(string all1, string all3, string i, string n_avv, string data_avv, string data_i, string ora_i, string data_f, string ora_f)
        {
            ...
            string consultazione = "Estrazione dati delle ore " + DateTime.Now.ToString("HH:mm") + " del " + DateTime.Now.ToString("dd/MM/yyyy");
            log.WriteToLog("ALL. A - Inizio scrittura bollettino, " + consultazione + ".", level.Info);
            filename = "AllegatoA.pdf";
            filenameTmp = "AllegatoAtmp.pdf";
            nomeFileCSV1 = all1;
            nomeFileCSV3 = all3;
            indiceBollettino = i;
            numAvviso = n_avv;
        ...
        private void CreaPDF()
        {
            try
            {
                ...
                using(fileCSV3 = new StreamReader(nomeFileCSV3)) 
                {
                    ...
                    document.Info.Title = "Bollettino di monitoraggio";
                    ...
                    {
                        FileInfo bInfoOld = new FileInfo(filenameTmp);
                        bInfoOld.IsReadOnly = false;
                        File.Delete(filenameTmp);
                    }

                    pdfRenderer.PdfDocument.Save(filenameTmp);

                    using (PdfDocument doc = PdfSharp.Pdf.IO.PdfReader.Open(filenameTmp, PdfDocumentOpenMode.Modify))
                    using (PdfDocument modello = PdfSharp.Pdf.IO.PdfReader.Open("./Modello.pdf", PdfDocumentOpenMode.Import))
                    {
                        // Apro il pdf appena creato con migradoc e il modello con i campi editabili per concatenarli

                        //for (int i = (doc.PageCount-1); i >= 0; i--)
                        //    modello.InsertPage(0, doc.Pages[i]);

                        if (modello.AcroForm.Elements.ContainsKey("/NeedAppearances"))
                            modello.AcroForm.Elements["/NeedAppearances"] = new PdfSharp.Pdf.PdfBoolean(true);
                        else
                            modello.AcroForm.Elements.Add("/NeedAppearances", new PdfSharp.Pdf.PdfBoolean(true));

                        //// Aggiunta del numero di pagina nel footer
                        //PdfTextField numpag = (PdfTextField)(modello.AcroForm.Fields["commento"]); 
                        //numpag.ReadOnly = false;
                        ////numpag.Value = new PdfString("Pagina " + (modello.PageCount).ToString() + " di " + (modello.Pages.Count).ToString());
                        //numpag.Value = new PdfString((modello.PageCount).ToString());
                        //numpag.ReadOnly = true;
                        // Salvo tutto in AllegatoA.pdf
                        //modello.Save(filename);     
                        doc.InsertPage(doc.PageCount, modello.Pages[0]);
                        doc.Save(filenameTmp);
                    ...
                    DisegnaCampiCompilabili(); // Il risultato sarà AllegatoA.pdf con i campicompilabili nell'header

                    File.Delete(filenameTmp); // Il pdf temporaneo si può eliminare 
        ...
        void DisegnaCampiCompilabili()
        {
            // Devo disegnare dei campi editabili sopra i campi "Data di emissione" e "Ora locale" nell'header del documento
            // Dimensioni dei campi da creare 
            int width = 80;
            int height = 13;

            using (iTextSharp.text.pdf.PdfReader reader = new iTextSharp.text.pdf.PdfReader(filenameTmp))
            using (PdfStamper stamper = new PdfStamper(reader, new FileStream(filename, FileMode.Create)))
            {

                //Seleziono tutte le pagine del documento appena aperto
                ...
                TextField p = new TextField(writer, new Rectangle(300, 733, (300 + width), (733 + height)), "dataemissione");
                p.Text = "gg.mm.aaaa";
                ...
                stamper.AddAnnotation(p.GetTextField(), 1); // Il campo che ho costruito va aggiunto alle coordinate impostate della pagina 1

                p = new TextField(writer, new Rectangle(470, 733, (470 + width), (733 + height)), "oralocale");
                p.Text = "hh:mm";
                p.Alignment = Element.ALIGN_LEFT;
                p.TextColor = BaseColor.BLACK;
                p.BorderWidth = 0;
                ...
                {
                    p = new TextField(writer, new Rectangle(500, 5, (500 + width), (5 + height)), ("numpag" + i.ToString()));
                    p.Text = "Pagina " + (i + 1).ToString() + " di " + numPages.ToString();
                    p.Alignment = Element.ALIGN_RIGHT;
                    p.TextColor = BaseColor.BLACK;
                    p.BorderWidth = 0;
                    p.BorderColor = BaseColor.WHITE;
                    p.FontSize = 6;
                    p.MaxCharacterLength = 20;
                    p.Options = BaseField.READ_ONLY; // Campo non modificabile
                    stamper.AddAnnotation(p.GetTextField(), i + 1); // Il campo che ho costruito va aggiunto alle coordinate impostate della pagina i+1
        ...
        void CreatePageHeader()
        {
            ...
            Image image = new Image("./LOGO RAS.tif");
            ...
            Image imagePC = new Image("./LOGO CIRCOLARE.gif");
            ...
            // Riga di spazio
            row = tableHeader.AddRow();
            row.Height = Unit.FromCentimeter(0.05);
        ...
        private void HeaderTableAll1()
        {
            ...
            // Definisco le colonne
            Column column = tableAll1.AddColumn("0.6cm");
            column = tableAll1.AddColumn("1.9cm");
        ...
        private void ContentTableAll1()
        {
            ...
            if (numBacini == 0)
            {
                ...
                log.WriteToLog("Allegato A - File " + nomeFileCSV1 + " vuoto!", level.Warning);
                Row row = tableAll1.AddRow();
                row.Format.Font.Bold = true;
                        ...
                        for (int s = 0; s < numStazioniPerBacino[b]; s++)
                        {
                            ...
                            for (int r = 0; r < numRighePerStazione; r++)
                            {
                                    ...
                                    ((elencoCampi[13] != "") && (Convert.ToDouble(elencoCampi[13]) >= 0.5)))
                                {
                                    ...
                                    // Nel secondo campo, oltre al nome della stazione, devo aggiungere l'informazione sull'ultimo dato disponibile,
        ...
        private void HeaderTableAll3()
        {
            ...
            column = tableAll3.AddColumn("2.3cm");
            column = tableAll3.AddColumn("2.5cm");
            column = tableAll3.AddColumn("2.5cm");
            column = tableAll3.AddColumn("1.3cm");
            column = tableAll3.AddColumn("0.7cm");
            column.Shading.Color = giallo;
            column = tableAll3.AddColumn("0.7cm");
            column.Shading.Color = giallo;
            column = tableAll3.AddColumn("0.7cm");
            column.Shading.Color = giallo;
            column = tableAll3.AddColumn("1.3cm");
            column = tableAll3.AddColumn("3.5cm");
        ...
        private void ContentTableAll3()
        {
            ...
            if (numBacini == 0)
            {
                ...
                log.WriteToLog("Allegato A - File " + nomeFileCSV3 + " vuoto!", level.Warning);
                Row row = tableAll3.AddRow();
                row.Format.Font.Bold = true;
                row.Cells[0].AddParagraph("Dati non presenti.");
                row.Cells[0].MergeRight = 10;
        ...
        void DefineStyles()
        {
            ...
            document.AddStyle("Comment", "Normal");
            Style comment = document.Styles["Comment"];
            comment.Font.Name = "Arial";
            comment.Font.Size = 24;
            comment.Font.Color = Colors.Gray;
            //comment.ParagraphFormat.Alignment = ParagraphAlignment.Center;
